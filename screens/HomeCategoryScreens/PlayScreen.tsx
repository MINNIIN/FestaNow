...
type PlayScreenNavigationProp = StackNavigationProp<any, 'Play'>;
...
const PlayScreen = ({ navigation }: Props) => {
  ...
  const fetchPlays = async () => {
    try {
      ...
      const playList = Array.isArray(list) ? list : [list];
      const filtered = playList.filter((item: any) =>
        item.genrenm?.includes('연극') 
  ...
  const renderItem = ({ item }: { item: any }) => (
    // 해당 콘서트를 눌렀을때 PlayDetail.tsx 화면으로 넘어가도록 해야함
    <TouchableOpacity
      onPress={() => navigation.navigate('PlayDetail', { id: item.mt20id })}
    >
    ...
    <View style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.titleContainer}>
          <Text style={styles.titleText}>진행 중인 연극</Text>
        </View>
