...
type ClassicScreenNavigationProp = StackNavigationProp<any, 'Classic'>;
...
const ClassicScreen = ({ navigation }: Props) => {
  ...
  const fetchClassics = async () => {
    try {
      ...
      const classicList = Array.isArray(list) ? list : [list];
  ...
  const renderItem = ({ item }: { item: any }) => (
    // 해당 콘서트를 눌렀을때 ClassicDetail.tsx 화면으로 넘어가도록 해야함
    <TouchableOpacity
      onPress={() => navigation.navigate('ClassicDetail', { id: item.mt20id })}
    >
